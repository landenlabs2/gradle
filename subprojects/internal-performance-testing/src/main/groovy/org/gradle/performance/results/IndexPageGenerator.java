...
public class IndexPageGenerator extends HtmlPageGenerator<ResultsStore> {
    ...
    private ScenarioBuildResultData queryExecutionData(ScenarioBuildResultData scenario) {
        ...
        List<? extends PerformanceTestExecution> currentCommitExecutions = recentExecutions.stream().filter(execution -> execution.getTeamCityBuildId() == scenario.getTeamCityBuildId()).collect(toList());
